;(function (){
	...
	var consumerKey = "570d49f2-d717-4dd8-ad1f-ed64d98697f3",
		consumerSecret = "4ec66d89-40a6-4ebd-965f-669d6a8a3e4c";
	...
	function processResponse(analyticData) {
		...
		log("<div/>Collection facets:");
		if (analyticData.facets) {
			for(var i=0, facet; facet=analyticData.facets[i]; i++) {
				log(SemantriaActiveSession.tpl("<div style='margin-left: 30px; margin-top: 10px;'/>{label} : {count}", facet));

				if (facet.attributes) {
					for(var j=0, attr; attr=facet.attributes[j]; j++) {
						log(SemantriaActiveSession.tpl("<div style='margin-left: 50px;'/>{label} : {count}", attr));
					}
				}
			}
		} else {
			log("<div style='margin-left: 30px;'/>No facets were extracted for this text");
		}
	}

	function receiveResponse(collectionId) {
		// As Semantria isn't real-time solution you need to wait some time before getting of the processed results
		// Wait ten seconds while Semantria process queued collection
		
		var analyticData = [];
		var timeout = setInterval(function() {
			log("Retrieving your processed results...");
			// Requests processed results from Semantria service
			var processedCollection = SemantriaActiveSession.getCollection(collectionId);

			console.log(processedCollection);
			if (processedCollection && processedCollection.status == 'PROCESSED') {
				log("</br>");
				clearInterval(timeout);
				processResponse(processedCollection);
			}
		}, 5000);
	}

	window.runTestApp = function() {
		log("<h1>Semantria Discovery mode demo.</h1>");
		...
		if (status == 202) {
			log("Collection #" + id + " queued successfully.");
		}
